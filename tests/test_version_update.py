...
from src.dashboard_app import app, _version_cache
...
class TestApiVersion:
    def test_returns_current_version(self, client):
        with patch("src.dashboard_app.urllib.request.urlopen", return_value=_make_pypi_response("99.0.0")):
            resp = client.get("/api/version")
        assert resp.status_code == 200
        data = resp.get_json()
        assert data["current"] == dashboard_app.__version__
        assert data["latest"] == "99.0.0"
        assert data["update_available"] is True

    def test_no_update_when_same_version(self, client):
        current = dashboard_app.__version__
        with patch("src.dashboard_app.urllib.request.urlopen", return_value=_make_pypi_response(current)):
            resp = client.get("/api/version")
        data = resp.get_json()
    ...
    def test_uses_cache_on_second_call(self, client):
        with patch("src.dashboard_app.urllib.request.urlopen", return_value=_make_pypi_response("5.0.0")) as mock_urlopen:
    ...
    def test_cache_expires_after_ttl(self, client):
        ...
        assert resp.get_json()["latest"] == "6.0.0"

    def test_pypi_failure_returns_current(self, client):
        with patch("src.dashboard_app.urllib.request.urlopen", side_effect=Exception("network error")):
            resp = client.get("/api/version")
        assert resp.status_code == 200
        data = resp.get_json()
        assert data["current"] == dashboard_app.__version__
    ...
    def test_pypi_timeout_returns_current(self, client):
        ...
        with patch("src.dashboard_app.urllib.request.urlopen", side_effect=urllib.error.URLError("timeout")):
            resp = client.get("/api/version")
        data = resp.get_json()
        assert data["update_available"] is False

    def test_response_shape(self, client):
        with patch("src.dashboard_app.urllib.request.urlopen", return_value=_make_pypi_response("1.2.3")):
            resp = client.get("/api/version")
        data = resp.get_json()
...
class TestApiUpdate:
    def test_returns_success(self, client):
        with patch("src.dashboard_app.subprocess.Popen") as mock_popen:
    ...
    def test_spawns_subprocess(self, client):
        ...
        cmd = args[0]
        assert cmd[0] == dashboard_app.sys.executable
        assert cmd[1] == "-c"
    ...
    def test_script_contains_server_pid(self, client):
        with patch("src.dashboard_app.subprocess.Popen") as mock_popen:
            with patch("src.dashboard_app.os.getpid", return_value=99999):
                client.post("/api/update")
    ...
    def test_windows_uses_detached_flags(self, client):
        with (
            patch("src.dashboard_app.subprocess.Popen") as mock_popen,
